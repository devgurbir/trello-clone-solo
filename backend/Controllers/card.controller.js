...
const updateTitle = async (req, res) => {
    try {
        const id = req.params.card_id;
        const data = await Card.findByIdAndUpdate(id, {title: req.body.title}, {returnDocument:'after'});
        if(!data){
...
const updateDescription = async (req, res) => {
    try {
        const id = req.params.card_id;
        const data = await Card.findByIdAndUpdate(id, {description: req.body.description}, {returnDocument:'after'});
        if(!data){
            return res.status(404).send({msg: "Card not found, please check ID"})
        }
        res.status(201).send({msg:"Description updated successfully", card: data})
...
const updateCover = async (req, res) => {
    try {
        const id = req.params.card_id;
        const data = await Card.findByIdAndUpdate(id, {cover: req.body.cover}, {returnDocument:'after'});
        if(!data){
            return res.status(404).send({msg: "Card not found, please check ID"})
        }
        res.status(201).send({msg:"Title updated successfully", card: data})
...
const updateLabels = async (req, res) => {
    try {
        const id = req.params.card_id;
        const data = await Card.findByIdAndUpdate(id, {labels: req.body.labels}, {returnDocument:'after'});
        if(!data){
            return res.status(404).send({msg: "Card not found, please check ID"})
        }
...
const addChecklist = async (req, res) => {
    ...
    try {
        const data = await Card.findByIdAndUpdate(req.params.card_id,
...
const deleteChecklist = async (req, res) => {
    try {
        let id = new mongoose.Types.ObjectId(req.body.id)
        const data = await Card.deleteOne(
            {"checklist.id": id }
        )
...
module.exports = {updateLabels, toggleChecklistItemStatus, createCard, getById, updateTitle, updateDescription, updateCover, addChecklist, addItemInChecklist,getChecklist, deleteChecklist }